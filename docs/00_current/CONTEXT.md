...
- Multilingual hardening ещё не завершён: P4 начинает configurable RU/UZ Latin/Cyrillic trigger foundation, полное hardening относится к P6.
...
- persisted `source_package_id` и hash-ready snapshot fields; финальный `context_snapshot_hash` относится к P5;
...
- полный P6 quality/acceptance baseline;
...
Завершён **P3-C — section policy routing**. Утверждён docs-level [P4 Targeted RAG RFC](P4_TARGETED_RAG_RFC.md); следующий implementation stage — **P4-A: source inventory**.

P4 использует deterministic backend planner без дополнительного LLM planning call, persistent immutable SourcePackage, exact source/chunk snapshots, backend-owned package statuses и RU/UZ trigger foundation. P4 хранит hash-ready данные, но финальный `context_snapshot_hash`, verdict binding и citation verification остаются P5.
...
- [P4_TARGETED_RAG_RFC.md](P4_TARGETED_RAG_RFC.md) — утверждённый implementation contract для P4.
