...
- P4 targeted RAG/source inventory/persistent source package;
- persisted `source_package_id`, hash-ready fields и финальный P5 `context_snapshot_hash`;
- P5 verified verdict/document gate;
- полный P6 quality/acceptance baseline из `TESTS_AND_RISKS.md`.

## 7. План до финала

### P4 — Targeted RAG / source inventory / source package

**Статус: implementation не начат; P4-RFC утверждён. Следующий этап — P4-A.**

Утверждённый contract находится в [P4_TARGETED_RAG_RFC.md](P4_TARGETED_RAG_RFC.md). P4 использует deterministic backend planner, UZ-only MVP и текущий lexical retriever; LLM planner, новая jurisdiction column и full taxonomy не входят в этап.

Порядок реализации:

1. **P4-A — Source inventory:** compact active+official metadata, stable sorting/limits, exclusions and tests.
2. **P4-B — RU/UZ triggers + deterministic planner:** canonical request schema, bounded chat context, validation/fallback, no planning LLM call.
3. **P4-C — Persistent SourcePackage:** migration/models, immutable exact source/chunk snapshots, hashes and backend-owned package status.
4. **P4-D — Invoke integration:** normal and verdict-skeleton modes use the package; verdict remains unconfirmed and document generation blocked.
5. **P4-E — Missing-source behavior:** empty/insufficient/failure paths remain distinct and cannot produce an unsupported confident answer.
6. **P4-F — Minimal API/UI visibility:** accurate package status and warnings without “verified/confirmed” wording.
7. **P4-G — Final verification/docs:** focused/full tests, applicable frontend build, smoke and PASS/PARTIAL/FAIL.

P4 prepares hash-ready snapshot data only. Final deterministic `context_snapshot_hash`, verdict binding and package-bound citation verification belong to P5.
...
- закрыть или привести к новой policy legacy Stage 4/5 active-verdict, `citation_verifier` и `structured_response` shortcut paths.

### P6 — Quality Gate

**Статус: не реализован полностью.**

Нужно реализовать и пройти полный P6 quality/acceptance baseline из `TESTS_AND_RISKS.md`, включая:
...
- [P4_TARGETED_RAG_RFC.md](P4_TARGETED_RAG_RFC.md) — утверждённый P4 implementation contract.
- [SECTION_GROUPS_AND_RAG_POLICY.md](../10_policies/SECTION_GROUPS_AND_RAG_POLICY.md) — canonical sections и правила маршрутизации.
- [PROMPT_SYSTEM_V1.md](../10_policies/PROMPT_SYSTEM_V1.md) — роли юристов и prompt/output policy.
- [RAG_WORKFLOW_V1.md](../10_policies/RAG_WORKFLOW_V1.md) — целевой targeted RAG workflow.
- [VERDICT_AND_DOCUMENT_POLICY_V1.md](../10_policies/VERDICT_AND_DOCUMENT_POLICY_V1.md) — eligibility verdict и document gates.
- [TESTS_AND_RISKS.md](TESTS_AND_RISKS.md) — verification baseline, required tests, риски и safety limits.
